...
def load_data(file_location):
    ...
    :param file_location -- location fo the file
    :return sparse array, largest user ID found, largest movie ID found
    """
    # load in data
...
def minhashing(sparse_data, max_user_id, max_movie_id, user_seed):
    ...
    data matrix
    :param sparse_data -- the sparse data set
    :param max_user_id -- the highest user ID
    :param max_movie_id -- the highest movie id
    :param user_seed -- the seed set by the user, to obtain repeatable results.
    :return the signature matrix and the amount of permutations used
    """
    print('minhashing in progress..')

    # We apply 100 randomly permutations
    ...
    for itPerm in range(total_permutation):
        ...
        for itInd in range(maxUserID):
            ar = new_sparse.indices[new_sparse.indptr[
                itInd]:new_sparse.indptr[itInd + 1]].min()
            signature_matrix[itPerm, itInd] = ar
...
def lsh_algorithm(signature_matrix, total_permutation):
    ...
    :param signature_matrix -- the signature matrix created with minhashing
    :param total_permutation -- the amount of permutations used with minhashing to calculate
                                the amount of rows in a band
    :return unique_set -- the unique candidate pairs with a similarity larger than 0.5 in the signature matrix
    """
    print("Running LSH..")
...
def jaccards_similarity(user1, user2, sparse_matrix):
    ...
    # print("Finding the jaccards similarity..")
...
if __name__ == '__main__':
    ...
    print("Run time in total : " + str(elapsed % 60) + " seconds")
